...
One should generally solve (privacy) problems as far down in the stack as possible.  Let’s talk about what that means, why it’s a good idea, how to do it, and where some of the pain points may be.

## Terminology

* **system**: application, product, or service
* **infrastructure**: a system that provides other systems with capabilities as **infrastructure**.  ([“Infrastructure: If Anything Exciting Happens, We’ve Done It Wrong”](https://www.youtube.com/watch?v=Wpzvaqypav8&t=1247s).)  
    * examples: storage, networking, data processing systems, server frameworks, libraries, system integrations
* **product**: a user-facing system (e.g. applications)

Infrastructure has **clients** (the systems that use it); products have **users** (the people that use it).

**Data-agnostic**[^1] infrastructure is not aware of the kinds of data it handles. (More precisely, it doesn’t handle data that’s privacy-relevant differently from other data.)  There can be valid engineering reasons for building infrastructure this way; it tends to be simpler and more general.  However, sometimes infrastructure is made data-agnostic to avoid responsibility for solving privacy problems (“we just handle data, it’s the client’s job to do it right”).[^2]

## Why (and when) should privacy be infrastructure’s problem?
...
Finally, because systems that scale in other ways (such as efficiently handling millions of queries per second or exabytes of data) **may create privacy problems at scale** if they don’t make a point of solving them.  Privacy problems that aren’t handled systematically across an organization will at best be re-solved repeatedly, inconsistently, and expensively.  Infrastructure and automation are a much better way to solve those problems than spreadsheets and a lot of human attention.

**Note**: if only one client needs (or will need) a solution to a privacy problem, that may indicate that the client should be solving that problem.
...
Infrastructure’s purpose for handling (privacy-relevant) data is usually only to provide the service (including debugging; more on this later).  However, if it has other purposes (such as building predictive models that incorporate data from multiple clients) those must be compatible with declared purpose limitations, and documented, just as they are for products.
...
* other controls (e.g. user-to-user sharing or blocking actions).
...
* complexity: how is the configuration performed?
    * simple UI (“enable encryption”)
    * creating/editing configuration files
    * writing code 

In descending order of preference (using a storage system as a motivating example):

### Zero configuration

Infrastructure automatically performs all relevant privacy functions for all clients; no configuration is possible (or necessary).
...
* data export (packaging and transmitting to the appropriate authorized user) is handled

### Good stance by default

Infrastructure does something (conservatively) appropriate for all clients, but some configuration is possible.
...
* clients must opt into specify a deletion timeline, and describe the data clients are responsible for deleting their own data to meet retention requirements
...
Infrastructure systems sometimes need to make client-visible changes.  This can place a large burden on clients, especially if the changes affect per-client configuration or otherwise impact privacy guarantees.

Infrastructure with open/public APIs may not know who all of its clients are; this can limit its ability to understand and address its clients’ needs (in general, as well as for privacy issues in particular), and make change management very difficult.
...
Infrastructure teams generally want to help solve their clients’ problems.  Privacy can and should work with these teams to help them to understand where there are opportunities for their infrastructure to solve privacy problems--and thus create value--for all of their clients.
